    ...
    <div class="block-header">
        <div class="row">
            <div class="col-lg-7 col-md-6 col-sm-12">
                <h2>Coverage Map Based on IMEI & Payload
                ...
                <div class="card">
                    ...
                    <div class="body">
                        ...
                        <script>
                            ...
                            <?php
                            foreach ($cimeipayload as $data) {
                            ?>
                                // Variable for getColor function
                                var rimei = [<?php echo json_encode($data['recommended_imei']); ?>];
                                var rpayload = [<?php echo json_encode($data['recommended_payload']); ?>];

                                // Set color based on condition
                                function getColor() {
                                    for (i = 0; i < rimei.length; i++) {
                                        if (rimei[i] == "Yes" && rpayload[i] == "Yes") {
                                            return {
                                                color: "Blue",
                                            }
                                        } else {
                                ...
                                var jsonTest = new L.GeoJSON.AJAX(["<?= base_url(); ?>/assets/plugins/leaflet/geojson/<?= $data['geojson'] ?>"], {
                                    onEachFeature: function popUp(f, l) {
                                        ...
                                        out.push("IMEI: " + "<?= round($data['percentage_percent'], 4) ?>" + " %" + " → " + "<?= $data['recommended_imei'] ?>");
                                        out.push("Payload: " + "<?= round($data['payload_MB'], 3) ?>" + " MB" + " → " + "<?= $data['recommended_payload'] ?>");
